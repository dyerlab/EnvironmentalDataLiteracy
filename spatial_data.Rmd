...
<div class="chapter_image"><img src="./media/RRC4_Winter_sunrise.jpg"></div>

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, message = FALSE, error = FALSE)
unloadNamespace("tidyverse")
if( "package:tidyverse" %in% search() ) detach("package:tidyverse",unload=TRUE,character.only = TRUE)
library(ggplot2)
theme_set( theme_bw() )
```
...
```{r ggplot_coords_points_projection}
...
```{r}
files <- c("./Data/alt.tif", "./Data/bio1.tif", "./Data/bio5.tif", "./Data/bio6.tif")
...
```{r extracting}
df <- raster::extract( bio_layers, pts)
df <- df[ !is.na(df[,1]),]
head(df)
```
...
```{r loreto_region, fig.cap="Extraction of region within polygon from the full Baja California raster."}
...
```{r plot_baja_all, message=FALSE, warning=FALSE}
...
```{r baja_plot_gConvex_hull, message=FALSE,warning=FALSE}
...
```{r baja_temp_precip_load}
library(raster)
baja_temp <- raster("./Data/bio1.tif")
baja_prec <- raster("./Data/bio12.tif")
```

And then extract the values from each of these layers into the `coords` data we already have set up.

```{r coords_temp_precip}
coords$elevation <- raster::extract( baja_california, coords[,c(2:3)] )
coords$mean_temp <- raster::extract( baja_temp,  coords[,c(2:3)] )
coords$mean_precip <- raster::extract( baja_prec , coords[,c(2:3)] )
coords[1:10,]
```

Two notes should be made here:  
1. About the temperature and precipitation values.  Temperature is denoted in tenths of a degree Celsius.  Though it does get quite hot at times, it does not average 188&deg;C at population 88!  Similarly, the units for precipitation are mm (or tenths of centimeters if you will...).  
2. The way that we called the `extract()` function here is with its 'fully specified namespace,' which is why it is `raster::extract()` instead of just `extract()`.  The reason for this is a bit complicated but it goes to show an important issue about functions and libraries in R.  The `raster` library has the `extract()` function nested within it and when you load up this library, it has 

```{r plot_elevation, warning=FALSE, message=FALSE}
library(ggrepel)
...
```{r load_click_data, echo=FALSE}
load("./Data/click_raster_example_pts.rda")
```
...
```{r plot_sptaial_points_cape_pts}
...
```{r extract_baja_polys}
data <- raster::extract( baja_california, polys )
length( data[[1]])
```
...
```{r baja_california, message=FALSE, warning=FALSE}
alt <- raster("./Data/alt.tif")
e <- extent( c(-115,-109,22,30) )
baja_california <- crop( alt, e )
baja_california
...
```{r baja_lcc, warning=FALSE,message=FALSE}
...
```{r mercator, message=FALSE, warning=FALSE}
library(ggplot2)
